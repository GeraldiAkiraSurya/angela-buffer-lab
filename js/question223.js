question223 = new Phaser.Scene('Question223');

question223.preload = function () {
    this.load.path = './assets/';
    this.load.image('emptyBeaker', 'question/emptyBeaker.png');
    this.load.image('purpleBeaker', 'question/purpleBeaker.png');

    this.load.image('223A', 'question/2.2.3A.png');
    this.load.image('223B', 'question/2.2.3B.png');
    this.load.image('223C', 'question/2.2.3C.png');
    this.load.image('223D', 'question/2.2.3D.png');
    this.load.image('223E', 'question/2.2.3E.png');
...
pada masing - masing larutan?`;
    ...
    //beaker-text
    A = this.add.image(middleX + 30, middleY - 300, '223A').setScale(1.2);
    // this.input.setDraggable(A);
    objectsArray.push(A);

    B = this.add.image(middleX + 630, middleY - 300, '223B').setScale(1.2);
    // this.input.setDraggable(B);
    objectsArray.push(B);

    C = this.add.image(middleX + 330, middleY - 60, '223C').setScale(1.2);
    // this.input.setDraggable(C);
    objectsArray.push(C);

    D = this.add.image(middleX + 30, middleY + 180, '223D').setScale(1.2);
    // this.input.setDraggable(D);
    objectsArray.push(D);

    E = this.add.image(middleX + 630, middleY + 180, '223E').setScale(1.2);
    ...
    cekJawabanBtn = createNextButton(this, 'CEK JAWABAN', () => {
        ...
        if (checkAnswerDraggable(answerArray, playerAnswerArray)) {
            ...
            showAnnouncementCorrectAnswer223(this, text);
        }
        else {
            //destory all objects
            destroyObject(objectsArray);
            ...
            showAnnouncementWrongAnswer223(this, text, clueText);
        }  
    ...
    //show pertanyaan
    showPertanyaan223(this, text);
}

question223.update = function () {
    
}

function showPertanyaan223(scene, text) {
...
function showClue223(scene, text) {
    var descriptionBox = scene.add.rectangle(scene.cameras.main.width / 2, scene.cameras.main.height / 2, scene.cameras.main.width / 2, scene.cameras.main.height * 3 / 4, 0x000000, 0.7);
...
function showAnnouncementCorrectAnswer223(scene, text) {
    var descriptionBox = scene.add.rectangle(scene.cameras.main.width / 2, scene.cameras.main.height / 2, scene.cameras.main.width / 2, scene.cameras.main.height * 3 / 4, 0x000000, 0.7);
...
function showAnnouncementWrongAnswer223(scene, text, clueText) {
    var descriptionBox = scene.add.rectangle(scene.cameras.main.width / 2, scene.cameras.main.height / 2, scene.cameras.main.width / 2, scene.cameras.main.height * 3 / 4, 0x000000, 0.7);
    var descBoxTopX = middleX - (descriptionBox.width / 2);
    var descBoxTopY = middleY - (descriptionBox.height / 2);
...
function startOver223(scene) {
    ...
    A = scene.add.image(middleX + 30, middleY - 300, '223A').setScale(1.2);

    B = scene.add.image(middleX + 630, middleY - 300, '223B').setScale(1.2);

    C = scene.add.image(middleX + 330, middleY - 60, '223C').setScale(1.2);

    D = scene.add.image(middleX + 30, middleY + 180, '223D').setScale(1.2);

    E = scene.add.image(middleX + 630, middleY + 180, '223E').setScale(1.2);
    ...
    cekJawabanBtn = createNextButton(scene, 'CEK JAWABAN', () => {
        ...
        if (checkAnswerDraggable(answerArray, playerAnswerArray)) {
            ...
            let text = "Bagus sekali, pilihan Anda benar.";
            showAnnouncementCorrectAnswer223(scene, text);
        }
        else {
