question2211 = new Phaser.Scene('Question2211');

question2211.preload = function () {
    this.load.path = './assets/';
    this.load.image('emptyBeaker', 'question/emptyBeaker.png');
    this.load.image('purpleBeaker', 'question/purpleBeaker.png');

    this.load.image('2211A', 'question/2.2.11A.png');
    this.load.image('2211B', 'question/2.2.11B.png');
    this.load.image('2211C', 'question/2.2.11C.png');
    this.load.image('2211D', 'question/2.2.11D.png');
    this.load.image('2211E', 'question/2.2.11E.png');
...
`;
    ...
    //beaker-text
    A = this.add.image(middleX + 30, middleY - 300, '2211A').setScale(1.2);
    // this.input.setDraggable(A);
    objectsArray.push(A);

    B = this.add.image(middleX + 630, middleY - 300, '2211B').setScale(1.2);
    // this.input.setDraggable(B);
    objectsArray.push(B);

    C = this.add.image(middleX + 330, middleY - 60, '2211C').setScale(1.2);
    // this.input.setDraggable(C);
    objectsArray.push(C);

    D = this.add.image(middleX + 30, middleY + 180, '2211D').setScale(1.2);
    // this.input.setDraggable(D);
    objectsArray.push(D);

    E = this.add.image(middleX + 630, middleY + 180, '2211E').setScale(1.2);
    ...
    cekJawabanBtn = createNextButton(this, 'CEK JAWABAN', () => {
        ...
        if (checkAnswerDraggable(answerArray, playerAnswerArray)) {
            ...
            showAnnouncementWrongAnswer2211(this, text, clueText);
        }  
    ...
    //show pertanyaan
    showPertanyaan2211(this, text);
}

question2211.update = function () {
    
}

function showPertanyaan2211(scene, text) {
...
function showClue2211(scene, text) {
    var descriptionBox = scene.add.rectangle(scene.cameras.main.width / 2, scene.cameras.main.height / 2, scene.cameras.main.width / 2, scene.cameras.main.height * 3 / 4, 0x000000, 0.7);
...
function showAnnouncementCorrectAnswer2211(scene, text) {
    var descriptionBox = scene.add.rectangle(scene.cameras.main.width / 2, scene.cameras.main.height / 2, scene.cameras.main.width / 2, scene.cameras.main.height * 3 / 4, 0x000000, 0.7);
    ...
    var nextBtn = createNextButton(scene, 'LANJUT', () => {
        ...
        //to question 12
        scene.scene.start('Question2212');
...
function showAnnouncementWrongAnswer2211(scene, text, clueText) {
    var descriptionBox = scene.add.rectangle(scene.cameras.main.width / 2, scene.cameras.main.height / 2, scene.cameras.main.width / 2, scene.cameras.main.height * 3 / 4, 0x000000, 0.7);
    var descBoxTopX = middleX - (descriptionBox.width / 2);
    var descBoxTopY = middleY - (descriptionBox.height / 2);
...
function startOver2211(scene) {
    ...
    A = scene.add.image(middleX + 30, middleY - 300, '2211A').setScale(1.2);

    B = scene.add.image(middleX + 630, middleY - 300, '2211B').setScale(1.2);

    C = scene.add.image(middleX + 330, middleY - 60, '2211C').setScale(1.2);

    D = scene.add.image(middleX + 30, middleY + 180, '2211D').setScale(1.2);

    E = scene.add.image(middleX + 630, middleY + 180, '2211E').setScale(1.2);
    ...
    cekJawabanBtn = createNextButton(scene, 'CEK JAWABAN', () => {
        ...
        if (checkAnswerDraggable(answerArray, playerAnswerArray)) {
            ...
            let text = "Bagus sekali, pilihan Anda benar.";
            showAnnouncementCorrectAnswer2211(scene, text);
        }
        else {
