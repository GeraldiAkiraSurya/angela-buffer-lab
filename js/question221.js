question221 = new Phaser.Scene('Question221');

question221.preload = function () {
    this.load.path = './assets/';
    this.load.image('emptyBeaker', 'question/emptyBeaker.png');
    this.load.image('purpleBeaker', 'question/purpleBeaker.png');

    this.load.image('221A', 'question/2.2.1A.png');
    this.load.image('221B', 'question/2.2.1B.png');
    this.load.image('221C', 'question/2.2.1C.png');
    this.load.image('221D', 'question/2.2.1D.png');
    this.load.image('221E', 'question/2.2.1E.png');
...
pada masing - masing larutan?`;
    ...
    //beaker-text
    A = this.add.image(middleX + 30, middleY - 300, '221A').setScale(1.2);
    // this.input.setDraggable(A);
    objectsArray.push(A);

    B = this.add.image(middleX + 630, middleY - 300, '221B').setScale(1.2);
    // this.input.setDraggable(B);
    objectsArray.push(B);

    C = this.add.image(middleX + 330, middleY - 60, '221C').setScale(1.2);
    // this.input.setDraggable(C);
    objectsArray.push(C);

    D = this.add.image(middleX + 30, middleY + 180, '221D').setScale(1.2);
    // this.input.setDraggable(D);
    objectsArray.push(D);

    E = this.add.image(middleX + 630, middleY + 180, '221E').setScale(1.2);
    ...
    cekJawabanBtn = createNextButton(this, 'CEK JAWABAN', () => {
        ...
        if (checkAnswerDraggable(answerArray, playerAnswerArray)) {
            ...
            showAnnouncementCorrectAnswer221(this, text);
        }
        else {
            //destory all objects
            destroyObject(objectsArray);
            ...
            let clueText = "Larutan amonia adalah elektrolit lemah, sedangkan larutan ammonium klorida adalah elektrolit kuat";
            showAnnouncementWrongAnswer221(this, text, clueText);
        }  
    ...
    //show pertanyaan
    showPertanyaan221(this, text);
}

question221.update = function () {
    
}

function showPertanyaan221(scene, text) {
...
function showClue221(scene, text) {
    var descriptionBox = scene.add.rectangle(scene.cameras.main.width / 2, scene.cameras.main.height / 2, scene.cameras.main.width / 2, scene.cameras.main.height * 3 / 4, 0x000000, 0.7);
...
function showAnnouncementCorrectAnswer221(scene, text) {
    var descriptionBox = scene.add.rectangle(scene.cameras.main.width / 2, scene.cameras.main.height / 2, scene.cameras.main.width / 2, scene.cameras.main.height * 3 / 4, 0x000000, 0.7);
...
function showAnnouncementWrongAnswer221(scene, text, clueText) {
    var descriptionBox = scene.add.rectangle(scene.cameras.main.width / 2, scene.cameras.main.height / 2, scene.cameras.main.width / 2, scene.cameras.main.height * 3 / 4, 0x000000, 0.7);
    var descBoxTopX = middleX - (descriptionBox.width / 2);
    var descBoxTopY = middleY - (descriptionBox.height / 2);
...
function startOver221(scene) {
    ...
    A = scene.add.image(middleX + 30, middleY - 300, '221A').setScale(1.2);

    B = scene.add.image(middleX + 630, middleY - 300, '221B').setScale(1.2);

    C = scene.add.image(middleX + 330, middleY - 60, '221C').setScale(1.2);

    D = scene.add.image(middleX + 30, middleY + 180, '221D').setScale(1.2);

    E = scene.add.image(middleX + 630, middleY + 180, '221E').setScale(1.2);
    ...
    cekJawabanBtn = createNextButton(scene, 'CEK JAWABAN', () => {
        ...
        if (checkAnswerDraggable(answerArray, playerAnswerArray)) {
            ...
            let text = "Bagus sekali, pilihan Anda benar.";
            showAnnouncementCorrectAnswer221(scene, text);
        }
        else {
